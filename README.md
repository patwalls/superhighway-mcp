...
Restart your client. The agent now has nineteen tools: web_search, news_search, scrape, geocode, nlp, email_verify, convert, qr, feed, sitemap, unfurl, fx (currency), dns (DNS records), hash (md5/sha/etc), color (hex/rgb/hsl), case (camel/snake/kebab/etc), base64 (encode/decode), jwt (decode JWT claims), and diff (text diff).
